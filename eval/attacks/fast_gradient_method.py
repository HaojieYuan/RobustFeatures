...
def fast_gradient_method(model_fn, x, eps, norm,
                         clip_min=None, clip_max=None, y=None, targeted=False, sanity_checks=False, transform=None):
  """
  ...
  if transform:
    x_infer = torch.stack([transform(img_adv) for img_adv in x])
  loss = loss_fn(model_fn(x_infer), y)
