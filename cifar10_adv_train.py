...
def normalize(x):
    return torch.stack([cifar10_preprocess_norm(img) for img in x])
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        adv_imgs = projected_gradient_descent(model, images.cuda(), 0.5, 0.05, 7, 2,
                                              clip_min=0., clip_max=1., y=target, targeted=False,
                                              rand_init=True, sanity_checks=True, transform=cifar10_preprocess_norm).detach()
        output_adv = model(normalize(adv_imgs))
